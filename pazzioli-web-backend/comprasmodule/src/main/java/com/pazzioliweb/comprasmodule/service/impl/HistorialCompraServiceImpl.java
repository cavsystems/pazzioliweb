...
public class HistorialCompraServiceImpl implements HistorialCompraService {
    ...
    public Page<HistorialCompraDTO> obtenerHistorial(Long bodegaId, Long proveedorId,
        ...
        List<HistorialCompraDTO> historialDTOs = ordenesPage.getContent().stream()
                .map(orden -> {
                    ...
                    dto.setProveedorNombre(orden.getProveedorId() != null ? orden.getProveedorId().toString() : null);
                    dto.setBodegaNombre(orden.getBodegaId() != null ? String.valueOf(orden.getBodegaId()) : null);
    ...
    @Override
    public Object obtenerEstadisticas(Long bodegaId, LocalDate fechaInicio, LocalDate fechaFin) {
        if (fechaInicio == null) {
            fechaInicio = LocalDate.now().minusMonths(1);
        }
        if (fechaFin == null) {
